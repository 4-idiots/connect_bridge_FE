...
export const ProjectRecruit = ({ member, setMember }) => {
  ...
  const handleInputChange = (e, index, sub) => {
    ...
    if (e.target.value === 'UI/UX 기획') {
      ...
      setMember({ ...member, salesBu: fieldMember.nsalesBu });
  ...
  const handleNeedPlus = recruitType => {
    if (recruitType === 'UI/UX 기획') {
      ...
      setFieldMember({ ...fieldMember, nsalesBu: fieldMember.nsalesBu + 1 });
      setMember({ ...member, salesBu: fieldMember.nsalesBu + 1 });
  ...
  const handleRemove = (index, recruitType) => {
    ...
    if (recruitType === 'UI/UX 기획') {
      ...
      setMember({ ...member, salesBu: 0, projectTotal: list });
      setFieldMember({ ...fieldMember, nsalesBu: 0 });
    } else if (recruitType === '전략/컨설팅') {
        ...
        {member.projectTotal.map((m, index) => {
          return (
            <div key={index} style={{ display: 'flex' }}>
              ...
              {m.sub === '영업' && (
                <>{genPopulation(fieldMember.nsalesBu, '영업')}</>
              )}
              {m.sub === '전략/컨설팅' && (
